export const Env = {
    TracksStorageAccountName: process.env.TracksStorageAccountName,
    TracksStorageAccountKey: process.env.TracksStorageAccountKey,
    TracksContainerName: process.env.TracksContainerName,
    CosmosConnectionString: process.env.CosmosConnectionString,
    CosmosDbName: process.env.CosmosDbName,
    CosmosTracksContainerName: process.env.CosmosTracksContainerName,
    DropboxAccessToken: process.env.DropboxAccessToken,
    TracksUploadSas: process.env.TracksUploadSas,
}
